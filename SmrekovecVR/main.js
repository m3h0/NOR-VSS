...
// Custom A-Frame component to scroll UVs on materials for flowing magma effect
AFRAME.registerComponent('uv-scroll', {
    schema: {
        speed: { type: 'vec2', default: { x: 0, y: 1 } }
    },
    tick: function (time, timeDelta) {
        if (!this.el.getObject3D('mesh')) return;
        const material = this.el.getObject3D('mesh').material;
        if (!material || !material.map) return;
        material.map.offset.x += this.data.speed.x * (timeDelta / 1000);
        material.map.offset.y += this.data.speed.y * (timeDelta / 1000);
    }
});

// Custom component to force maximum texture quality for panoramas
    ...
    init: function () {
        this.el.addEventListener('loaded', () => {
            ...
            if (mesh && mesh.material && mesh.material.map) {
                ...
                // Use heavy linear filtering to blur out hard pixelation edges
                map.magFilter = THREE.LinearFilter;
                map.minFilter = THREE.LinearMipMapLinearFilter;

                // Get max anisotropy supported by the device (usually 16x)
                const maxAnisotropy = this.el.sceneEl.renderer.capabilities.getMaxAnisotropy();
                map.anisotropy = maxAnisotropy;

                // Generate mipmaps to create naturally softer versions of the image
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateLavaCracks() {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            cracksHtml += `
                <a-plane class="lava-crack" position="${x.toFixed(2)} -4.9 ${z.toFixed(2)}" rotation="-90 ${rotY.toFixed(2)} 0" width="${width.toFixed(2)}" height="${length.toFixed(2)}" material="src: #magma; color: #ffffff; emissive: #ff2200; emissiveIntensity: 0.1" uv-scroll="speed: 0 -0.2" animation__glow="property: material.emissiveIntensity; to: ${2 + Math.random() * 2}; dur: ${1000 + Math.random() * 1000}; startEvents: trigger-cracks; easing: easeOutQuad" shadow="receive: true"></a-plane>
            `;
        }
        return cracksHtml;
    }
    ...
    function generateClouds() {
        ...
        for (let i = 0; i < 40; i++) {
            ...
            const driftDuration = 40000 + Math.random() * 20000;
            cloudsHtml += `
                <a-entity position="${x.toFixed(2)} ${y.toFixed(2)} ${z.toFixed(2)}" scale="${scale.toFixed(2)} ${scale.toFixed(2)} ${scale.toFixed(2)}" animation="property: position; to: ${(x + 20).toFixed(2)} ${y.toFixed(2)} ${(z + 20).toFixed(2)}; dur: ${Math.round(driftDuration)}; dir: alternate; loop: true; easing: linear">
                    <a-sphere position="0 0 0" radius="5" material="color: #9ca3af; shader: flat; transparent: true; opacity: ${opacity.toFixed(2)}" shadow="cast: false"></a-sphere>
                    <a-sphere position="-3 -1 2" radius="4" material="color: #6b7280; shader: flat; transparent: true; opacity: ${opacity.toFixed(2)}" shadow="cast: false"></a-sphere>
                    <a-sphere position="4 1 -2" radius="6" material="color: #9ca3af; shader: flat; transparent: true; opacity: ${opacity.toFixed(2)}" shadow="cast: false"></a-sphere>
                    <a-sphere position="2 -2 3" radius="4.5" material="color: #d1d5db; shader: flat; transparent: true; opacity: ${opacity.toFixed(2)}" shadow="cast: false"></a-sphere>
                </a-entity>
            `;
        }
        return cloudsHtml;
        ...
        const sceneHtml = `
            <a-scene embedded renderer="colorManagement: true; physicallyCorrectLights: true; antialias: true;" style="width: 100vw; height: 100vh;" vr-mode-ui="enabled: true" fog="type: exponential; color: #170A04; density: 0.04">
                ...
                <a-assets>
                    ...
                    <img id="rock" src="https://images.unsplash.com/photo-1525926477800-7a3ddfaee5a0?q=80&w=1024&auto=format&fit=crop&t=${timestamp}" crossorigin>
                    <img id="magma" src="https://images.unsplash.com/photo-1616790875938-160fa1f974eb?q=80&w=1024&auto=format&fit=crop&t=${timestamp}" crossorigin>
                    <img id="smrekovec-pano" src="/smrekovec.jpg" crossorigin="anonymous">
                    <!-- Using locally generated professional geological images to guarantee 100% load success without CORS blocks -->
                    <img id="andesite-rock" src="/andesite.png">
                    <img id="tectonic-plates" src="/tectonic.png">
                ...
                <a-entity id="magma-light" light="type: point; color: #ff2200; intensity: 4; distance: 350; castShadow: true" position="0 40 -60"></a-entity>

                <!-- High Altitude Clouds -->
                <a-entity id="clouds-layer">
${generateClouds()}
                </a-entity>

                <!-- Magma Bloom Core (Fakes post-processing bloom via additive blending rendering) -->
                <a-sphere position="0 -10 -70" radius="55" material="color: #ff3300; shader: flat; transparent: true; opacity: 0.15; blending: additive" side="back"></a-sphere>

                <!-- Immersive Ambient Audio -->
                <a-sound src="https://cdn.aframe.io/basic-guide/audio/backgroundnoise.wav" autoplay="true" loop="true" volume="0.08" position="0 2 0"></a-sound>

                <!-- Volcanic Ash & Embers Particles -->
                <a-entity position="0 5 -15" custom-particles="count: 2000; color: #ff5500; size: 0.15; speedY: -2; spreadX: 40; spreadZ: 40; maxAge: 8; opacity: 0.8" animation="property: position; to: 0 4 -15; dir: alternate; loop: true; dur: 4000"></a-entity>
...
${generatePterodactyls()}
                ...
                <!-- Floating Educational Card 1: Smrekovec Today -->
                ...
                <!-- Floating Educational Card 2: Andesite -->
                <a-entity position="4 2.5 -4" look-at="#camera">
                    <a-plane width="2.5" height="3.2" color="#1a110d" material="roughness: 0.8" shadow="cast: true"></a-plane>
                    
                    <!-- Image -->
                    <a-image src="#andesite-rock" width="2.2" height="1.5" position="0 0.6 0.05"></a-image>

                    <!-- Text -->
                    <a-text value="ANDEZITNE KAMNINE" align="center" position="0 -0.4 0.05" width="2.5" color="#f46a25" font="kelsonsans" shadow="cast: true"></a-text>
                    <a-text value="Ta temna vulkanska kamnina je nastala s pocasnim ohlajanjem magme. Je prepoznavni kamen celotnega gorovja Smrekovec." 
                            align="center" position="0 -0.7 0.05" width="2.2" baseline="top" wrap-count="32" color="#dddddd" shadow="cast: true"></a-text>
                </a-entity>
                
                <!-- Floating Educational Card 3: Tectonics -->
                <a-entity position="-8 3.8 -3" look-at="#camera">
                    <a-plane width="2.5" height="3.2" color="#1a110d" material="roughness: 0.8" shadow="cast: true"></a-plane>
                    
                    <!-- Image -->
                    <a-image src="#tectonic-plates" width="2.2" height="1.5" position="0 0.6 0.05"></a-image>

                    <!-- Text -->
                    <a-text value="TEKTONSKI PREMIKI" align="center" position="0 -0.4 0.05" width="2.5" color="#f46a25" font="kelsonsans" shadow="cast: true"></a-text>
                    <a-text value="Smrekovec se je dvignil z morskega dna zaradi trka afriske in evrazijske tektonske plosce pred 23 milijoni let." 
                            align="center" position="0 -0.7 0.05" width="2.2" baseline="top" wrap-count="32" color="#dddddd" shadow="cast: true"></a-text>
                </a-entity>
                ...
                <a-entity position="0 0.2 -5">                    
                    ...
                    <!-- Portal Orb to 360 Photo -->
                    <a-entity position="3.5 1.5 -3.5">
                        <!-- Use the pano image on the orb itself for a blurred preview -->
                        <a-sphere id="portal-orb" class="clickable" radius="0.45" material="src: #smrekovec-pano; roughness: 0.5; metalness: 0; shader: flat" high-res-texture shadow="cast: false"
                            ...
                            animation__spin="property: rotation; to: 0 360 0; loop: true; dur: 15000; easing: linear"
                            animation__float="property: position; to: 0 0.1 0; dir: alternate; loop: true; dur: 3000; easing: easeInOutSine">
                        </a-sphere>
                        <!-- Moved text up so it hovers clearly above the orb instead of clipping through -->
                        <a-text value="360 OGLED SMREKOVCA PO SNEGU\n(Glej 2 sekundi)" align="center" position="0 0.8 0" scale="0.8 0.8 0.8" color="#f46a25" font="kelsonsans" shadow="cast: true"></a-text>
                    </a-entity>
                </a-entity>

                <!-- Portal Environment for 360 Photo -->
                <a-entity id="portal-env" position="0 5000 0">
                    <!-- High definition curved image, scales mapping to image proportions natively -->
                    <a-curvedimage src="#smrekovec-pano" radius="10" height="10" theta-length="360" theta-start="0" rotation="0 -90 0" position="0 1.6 0" material="shader: flat; fog: false; npot: true; side: back" high-res-texture shadow="cast: false"></a-curvedimage>
                    <!-- Create a dark spherical background to hide the harsh black void if they look straight up/down -->
                    <a-sky color="#0a1526" material="fog: false" radius="20"></a-sky>
                    <a-sphere id="return-orb" class="clickable" radius="0.4" position="0 1.6 -3" material="color: #ff3300; emissive: #ffaa00; emissiveIntensity: 1; roughness: 0.1; fog: false" shadow="cast: false"
                        animation__hover="property: scale; to: 1.1 1.1 1.1; startEvents: mouseenter; dur: 200"
                        animation__leave="property: scale; to: 1 1 1; startEvents: mouseleave; dur: 200"
                        animation__float="property: position; to: 0 1.8 -3; dir: alternate; loop: true; dur: 2000; easing: easeInOutSine">
                        <a-text value="NAZAJ NA VULKAN\\n(Glej 2 sekundi)" align="center" position="0 1 0" color="#fff" side="double"></a-text>
                    </a-sphere>
                </a-entity>
                ...
                <a-entity id="shake-container" position="0 0 0">
                    <a-entity id="rig" position="0 1.6 -1">
                        <a-camera id="camera" look-controls="enabled: true" wasd-controls="acceleration: 20; enabled: true">
        ...
        setTimeout(() => {
            ...
            const returnOrb = document.getElementById('return-orb');
            const cameraEl = document.getElementById('camera');

            if (portalOrb) {
                portalOrb.addEventListener('click', () => {
                    rig.setAttribute('position', '0 5000 0');
                    // Disable fog so we can see the 360 photo, and hide the main environment
                    scene.setAttribute('fog', 'density: 0');
                    // Lock player into place, allowing only head turning
                    cameraEl.setAttribute('wasd-controls', 'enabled: false');
                });
            }
            if (returnOrb) {
                returnOrb.addEventListener('click', () => {
                    rig.setAttribute('position', '0 1.6 -1');
                    // Re-enable original fog density
                    scene.setAttribute('fog', 'density: 0.04');
                    // Re-enable player movement using WASD
                    cameraEl.setAttribute('wasd-controls', 'enabled: true; acceleration: 20');
                });
            }
    ...
    function showIntroVideo() {
        ...
        enterGameBtn.classList.remove('visible');
        introVideo.currentTime = 0;
        introVideo.play().catch(() => {
            // Autoplay blocked by browser — show button immediately
            enterGameBtn.classList.add('visible');
        });
    }

    function hideIntroAndEnterVR() {
    ...
    if (enterGameBtn) {
        enterGameBtn.addEventListener('click', hideIntroAndEnterVR);
    }

    if (skipIntroBtn) {
        skipIntroBtn.addEventListener('click', hideIntroAndEnterVR);
    }

    // Ensure event listeners point to the intro video first, then VR

    if (enterVrNav) enterVrNav.addEventListener('click', showIntroVideo);
    if (enterVrCta) enterVrCta.addEventListener('click', showIntroVideo);
